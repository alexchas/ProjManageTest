...
ItemListModel::ItemListModel(QListView *view, DataSet *dataset): QAbstractItemModel() {
    m_view = view;
    m_dataset = dataset;
...
DataSetPar *ItemListModel::appendItem(Item *item) {

    int newIndex = m_items.length();
    beginInsertRows(QModelIndex(), newIndex, newIndex);

    m_items.append( new DataSetPar(nullptr) );
    DataSetPar *i = m_items.at(newIndex);
    i->setItem(item);
...
bool ItemListModel::insertRows(int position, int rows, const QModelIndex &parent) {
    ...
    for(int i = 0; i < rows; i++)
        m_items.insert(position + i, new DataSetPar(nullptr));
...
Item *ItemListModel::itemFromIndex(QModelIndex index) {
    if(!index.isValid())
        return nullptr;
    DataSetPar *item = static_cast<DataSetPar*>(index.internalPointer());
    return item->item();

}

Model::Model(QSharedPointer<Entity> entity, QObject *parent) : QStyledItemDelegate(parent)
{
    m_entity = entity;
}
...
void Model::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
QSize Model::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
